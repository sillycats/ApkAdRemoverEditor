#!/bin/bash
#===============================================================================
# APK 编译环境一键恢复脚本（配置文件版，需 APK_env.conf）
# 用途：从 save_env.sh 生成的配置文件一键恢复 APK 编译环境
# 使用：bash restore_env.sh [--skip-sdk] [--skip-gradle-cache] [--skip-mirrors]
#
# 国内镜像源（按速度排序，自动切换）：
#   Gradle 发行版:  腾讯云 / 华为云 / 官方
#   Android SDK:    Google 官方 / 腾讯云 / 华为云
#   Maven 仓库:     阿里云 / 腾讯云 / 华为云 / jitpack
#===============================================================================
...
SKIP_SDK=false
SKIP_GRADLE_CACHE=false
SKIP_MIRRORS=false
for arg in "$@"; do
    case $arg in
        --skip-sdk) SKIP_SDK=true ;;
        --skip-gradle-cache) SKIP_GRADLE_CACHE=true ;;
        --skip-mirrors) SKIP_MIRRORS=true ;;
        --help|-h)
            echo "用法: bash restore_env.sh [--skip-sdk] [--skip-gradle-cache] [--skip-mirrors]"
            echo ""
            echo "参数说明:"
            echo "  --skip-sdk           跳过 Android SDK 安装"
            echo "  --skip-gradle-cache  跳过 Gradle 缓存恢复"
...
# ──────────────────────────────────────────────
if [ ! -f "$ENV_FILE" ]; then
    log_err "未找到环境配置文件: $ENV_FILE"
    log_err "请先在原环境运行 save_env.sh 生成配置"
    log_err "或使用 build_env.sh 进行独立环境配置（无需配置文件）"
    exit 1
fi

log_info "[1/10] 读取环境配置..."
source "$ENV_FILE"
echo "  项目目录:  $PROJECT_DIR"
echo "  Gradle:    v${GRADLE_VERSION:-8.5}"
echo "  NDK:       v${NDK_VERSION:-21.4.7075529}"
echo ""

# ──────────────────────────────────────────────
# Step 2: 检查/安装 JDK
# ──────────────────────────────────────────────
log_info "[2/10] 检查 JDK 环境..."
JAVA_MAJOR=""
...
# ──────────────────────────────────────────────
# Step 3: 检查/恢复 Gradle（多镜像源下载）
# ──────────────────────────────────────────────
GRADLE_VERSION="${GRADLE_VERSION:-8.5}"
log_info "[3/10] 检查 Gradle $GRADLE_VERSION..."
GRADLE_INSTALL_DIR="/opt/gradle-${GRADLE_VERSION}"
GRADLE_BIN="$GRADLE_INSTALL_DIR/bin/gradle"

if [ -x "$GRADLE_BIN" ]; then
    log_ok "  Gradle 已就绪: $GRADLE_BIN"
else
    log_warn "  Gradle 未找到，正在从国内镜像下载安装..."
    GRADLE_DIST="gradle-${GRADLE_VERSION}-bin.zip"

    if download_from_mirrors "/tmp/${GRADLE_DIST}" "$GRADLE_DIST" "${GRADLE_MIRRORS[@]}"; then
        mkdir -p "$GRADLE_INSTALL_DIR"
        unzip -q "/tmp/${GRADLE_DIST}" -d /opt/
        rm -f "/tmp/${GRADLE_DIST}"
        chmod +x "$GRADLE_BIN"
        log_ok "  Gradle 安装完成: $GRADLE_BIN"
    else
        log_err "  所有镜像源均下载失败，请手动安装 Gradle $GRADLE_VERSION"
        exit 1
    fi
fi
echo ""
...
SDK_DIR="${ANDROID_SDK:-/opt/android-sdk}"

log_info "[4/10] 检查 Android SDK..."

if $SKIP_SDK; then
    log_info "  (--skip-sdk) 跳过 SDK 检查"
else
    if [ -d "$SDK_DIR/platforms/$PLATFORM_VERSION" ] && [ -d "$SDK_DIR/build-tools/$BUILD_TOOLS_VERSION" ]; then
        ...
        if [ -z "$SDKMANAGER" ]; then
            log_info "  下载 Android SDK command-line tools（多镜像源）..."
...
# ──────────────────────────────────────────────
# Step 5: 生成 local.properties
# ──────────────────────────────────────────────
log_info "[5/10] 生成 local.properties..."
...
# ──────────────────────────────────────────────
# Step 5a: 配置 Gradle 代理（检测环境变量）
# ──────────────────────────────────────────────
log_info "[5a/10] 配置 Gradle 代理..."
...
# ──────────────────────────────────────────────
# Step 6: 配置 Gradle Wrapper（使用最快镜像）
# ──────────────────────────────────────────────
log_info "[6/10] 配置 Gradle Wrapper..."
...
#APK restore_env.sh v2.0 auto-generated
...
EOF
log_ok "  Gradle Wrapper 已配置 (镜像: $(basename $SELECTED_GRADLE_MIRROR))"
...
# ──────────────────────────────────────────────
# Step 7: 配置仓库镜像（国内多镜像源）
# ──────────────────────────────────────────────
log_info "[7/10] 配置仓库镜像（阿里云/腾讯云/华为云）..."
...
# ──────────────────────────────────────────────
# Step 8: 恢复 Gradle 缓存（如有）
# ──────────────────────────────────────────────
log_info "[8/10] 检查 Gradle 缓存..."
CACHE_FILE="$SCRIPT_DIR/gradle_cache.tar.gz"

if $SKIP_GRADLE_CACHE; then
    log_info "  (--skip-gradle-cache) 跳过缓存恢复"
else
    if [ -f "$CACHE_FILE" ]; then
        log_info "  发现缓存包，正在恢复..."
        tar -xzf "$CACHE_FILE" -C "$HOME" 2>/dev/null && log_ok "  Gradle 缓存已恢复" || log_warn "  缓存恢复失败（可忽略）"
    else
        log_info "  未找到缓存包，首次编译将自动下载依赖"
    fi
fi
echo ""
...
if [ -d "$SDK_DIR/platforms/$PLATFORM_VERSION" ]; then
    log_ok "  Android SDK: $PLATFORM_VERSION"
else
    log_err "  Android SDK: 未就绪"
    ERRORS=$((ERRORS + 1))
fi
...
# CMake
if [ -d "$SDK_DIR/cmake/$CMAKE_VERSION" ]; then
...
if [ -d "$SDK_DIR/build-tools/$BUILD_TOOLS_VERSION" ]; then
    log_ok "  build-tools: $BUILD_TOOLS_VERSION"
else
    log_err "  build-tools: 未就绪"
    ERRORS=$((ERRORS + 1))
fi

# 项目文件
if [ -f "$PROJECT_DIR/build.gradle" ]; then
    log_ok "  项目文件: build.gradle"
else
    log_err "  项目文件: build.gradle 未找到"
    ERRORS=$((ERRORS + 1))
fi
...
if [ $ERRORS -eq 0 ]; then
    echo -e "${GREEN}  环境恢复成功！${NC}"
    echo ""
    echo "  编译命令:"
    echo "    cd $PROJECT_DIR"
    echo "    $GRADLE_BIN assembleRelease"
    echo "    # 或使用 wrapper: ./gradlew assembleRelease"
    echo ""
    echo "  APK 输出:"
    ...
    echo ""
    echo "  镜像源:"
    echo "    Gradle:  腾讯云 / 华为云 / 官方"
    echo "    Maven:   阿里云 / 腾讯云 / 华为云 / jitpack"
    echo "============================================"
else
    echo -e "${RED}  环境恢复不完整（$ERRORS 项缺失）${NC}"
    echo "  请手动安装缺失组件后重试"
    echo "============================================"
    exit 1
fi